import type { DidKeys, DidUser, DidDocument } from '@/types/did'
import type { RuntimeParams } from '@/types/messaging'

class IonDB {
    private static dbPromise: Promise<IDBDatabase>
    private static requiredStoreNames = ['users', 'keys', 'documents']
    static async getDB() {
...
const get = async (storeName: string, id: IDBValidKey) => {
    ...
    const store = tx.objectStore(storeName)
    const req = id === 'ALL' ? store.getAll() : store.get(id)
...
export const getUser = async ({ did }: RuntimeParams) => {
    if (did) return await get('users', did) as DidUser
    return await get('users', 'ALL') as DidUser[]
}
...
export const updateDocument = async ({ did, patch }: RuntimeParams) => {
    ...
    for (const pk of publicKeysToAdd) {
        if (buffer?.filter(ex => ex.id === pk.id).length) throw new Error('Public Key Already Exists')
        buffer?.push(pk)
    }
